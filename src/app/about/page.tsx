...
const values = [
  { title: "Precision", description: "Drawings are followed to the millimetre, not treated as suggestions." },
  { title: "Transparency", description: "Clients see cost, timeline, and progress in writing — every week." },
  { title: "Craft", description: "Materials and detailing chosen for how they age, not just how they photograph." },
];
  ...
  return (
    <>
      <section className="relative flex min-h-[70vh] items-end bg-ink text-canvas">
        ...
        <div className="container-px relative mx-auto pb-24 pt-40">
      ...
      <section className="py-24 md:py-32">
        <div className="container-px mx-auto">
          <SectionHeading eyebrow="Studio Values" title="What we won't compromise on." />
          <div className="mt-14 grid grid-cols-1 gap-10 md:grid-cols-3">
            {values.map((value, i) => (
              <RevealOnScroll key={value.title} delay={i * 0.1} className="border-t border-line pt-6 dark:border-line-dark">
                <span className="font-mono text-xs text-gold">{String(i + 1).padStart(2, "0")}</span>
                <h3 className="mt-3 font-display text-2xl text-ink dark:text-canvas">{value.title}</h3>
                <p className="mt-3 text-concrete">{value.description}</p>
              </RevealOnScroll>
            ))}
          </div>
        </div>
